...
class Aklz:
    ...
    def decompress(self, file_in: bytearray) -> bytearray:
        ...
        if not file_sig == self._FILE_SIGNATURE:
            return bytearray(file_in)

        file_in_size = len(file_in)
        file_out_size = int.from_bytes(bytes=file_in[file_in_ptr:file_in_ptr + 4], byteorder=self._ENDIAN)
        file_in_ptr += 4
        cur_out_size = 0
        ...
        while file_in_ptr < file_in_size and cur_out_size < file_out_size:
            ...
            for i in range(8):
                ...
                if flag:
                    ...
                    match_len = (file_in[file_in_ptr + 1] & self._MATCH_SIZE) + self._MATCH_BEG
                    file_in_ptr += 2
                    match_loc = raw_match_pos
                    num8 = int(cur_out_size / self._BUFFER_SIZE)

                    for j in range(num8):
                        if raw_match_pos + j * self._BUFFER_SIZE < cur_out_size:
                            match_loc += self._BUFFER_SIZE
    ...
    def compress(self, file_in: bytearray) -> bytearray:

        if file_in[:len(self._FILE_SIGNATURE)] == self._FILE_SIGNATURE:
            return file_in
        ...
        file_out += self._FILE_SIGNATURE
        file_out_size = bytearray(file_size.to_bytes(length=4, byteorder=self._ENDIAN))
        ...
        while cur_byte < file_size:
            ...
            for i in range(8):
                ...
                if array2[1] > 0:
                    ...
                    value: bytes = (num6 & 0xFF).to_bytes(length=1, byteorder=self._ENDIAN)
                    value2: bytes = (num5 | ((num6 & 0xF00) >> 4)).to_bytes(length=1, byteorder=self._ENDIAN)
                    cur_bytes += value
                    cur_bytes += value2
                    ...
                    cur_bytes += array[cur_byte].to_bytes(length=1, byteorder=self._ENDIAN)
            ...
            file_out += flag_byte.to_bytes(length=1, byteorder=self._ENDIAN)
            file_out += cur_bytes
...
if __name__ == '__main__':
    filename = os.path.join(os.path.curdir, "test_file.test")
    ...
    aklz = Aklz()
    print('Decompiling')
    file_de = aklz.decompress(file_in=file)

    print('Recompiling')
    file_de_re = aklz.compress(file_in=file_de)

    print('Decompiling the recompiled file')
    file_de_re_de = aklz.decompress(file_in=file_de_re)

    decomp_same = True
    if len(file_de) != len(file_de_re_de):
        decomp_same = False

    for i in range(len(file_de)):
        if file_de[i] != file_de_re_de[i]:
            decomp_same = False

    if decomp_same:
        print('File successfully decompression, recompression, and decompression again.')
    else:
        print('File was not successfully decompression, recompression and decompression again, Please check for errors')

    if len(file) == len(file_de_re):
        print('File size was successfuly preserved during recompression')
        if decomp_same:
            print('This module appears to be intact')
